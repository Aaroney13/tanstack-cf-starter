...
BetterAuth's API is mounted at `/api/auth/*` (`src/routes/api/auth/$.ts`). `/dashboard` (`src/routes/_authed/dashboard.tsx`) is the example guarded route: it reads the session from the `_authed` layout's route context, and signed-out visitors are redirected to `/`.
...
5. Set `BETTER_AUTH_URL` in `wrangler.jsonc` / `wrangler.dev.jsonc` `vars` to each Worker's public URL
6. `DATABASE_URL=<prod Neon URL> pnpm db:migrate` to apply migrations to the prod database
7. `pnpm deploy:dev` or `pnpm deploy:prod`
